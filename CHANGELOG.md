...
Released to PyPI alongside the same-day packaging and documentation fixes that
became 1.0.2 and 1.0.3. No changelog entry was written at the time and the
repository carries no tag for it, so the specific changes are not recoverable;
this entry exists so the released versions and the changelog agree.
